...
class GaussianDistribution(object):
    ...
    def sample(self, store):
        ...
        if store:               # log posterior  - log prior
            self._cum_kl_div += (self.log_prob(self._last_sample) - self._prior.log_prob(self._last_sample))
...
#
    ...
    def clip(input, to=(0.0001,0.9999)):
        return torch.clamp(tensor, to[0], to[1])
...
class Linear(BaseLayer):
    ...
    """
    def __init__(self, in_features, out_features, mu = 0, logstd1 = -1, logstd2 = -2, pi = 0.5, type = ModelType.BBB):
