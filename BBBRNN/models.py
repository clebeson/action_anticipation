...
class BActAnticipationModel(nn.Module):
    ...
    def __init__(self, data_type, input_size, output_size, hidden_dim,n_layers, stochastic = True, rnn_dropout = 0.5, fc_dropout = 0.5, logstd1 = -2, logstd2 = -4, pi = 0.5 ):
        super(BActAnticipationModel, self).__init__()
        type = bl.ModelType.MC_DROP if stochastic else bl.ModelType.DET
        ...
        self.fc = bl.Linear(self.hidden_dim, self.output_size, dropout = fc_dropout, **linear_args)
    ...
    def set_dropout(self, value, training=True):
        self.dropout = value
        self.movement.set_dropout(value)
        self.context.set_dropout(value)
        self.lstm.set_dropout(value)
        self.fc_combine.set_dropout(value)
        self.fc.set_dropout(value)
...
def train_LSTM( train_data, params):
    device = torch.device('cuda:{}'.format(params["gpu"]) if torch.cuda.is_available() else 'cpu')
    model = BActAnticipationModel( data_type =  params["data_type"], 
                                   input_size =  params["input_size"], 
                                   output_size =  params["num_classes"], 
                                   hidden_dim =  params["hidden_dim"], 
                                   n_layers =  params["n_layers"],  
                                   stochastic =  True if params["model"].startswith('BLSTM') else False,
                                   rnn_dropout = params["rnn_drop"], 
                                   fc_dropout = params["fc_drop"])
    ...
    #Hyperparameters
    epochs = params["epoch"]
    batch = params["batch_size_{}".format(params["num_classes"])]
