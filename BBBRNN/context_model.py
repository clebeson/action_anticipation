...
class ContextModel(nn.Module):
    def __init__(self, mu = 0, logstd1 = -1, logstd2 = -2, pi = 0.5, type = bl.ModelType.BBB):
        ...
        self.enc_ball = bl.Linear(b, 16, **linear_args)
        self.enc_head = bl.Linear(h, 16,**linear_args)
        self.fc_combine = bl.Linear(32, 16,**linear_args)
        
        self.encoder_ball_position = nn.Sequential(self.enc_ball, nn.ReLU())
        self.encoder_head = nn.Sequential(self.enc_head, nn.ReLU())
    ...
    def get_kl(self):
        kl = self.enc_ball.get_kl()
        kl += self.enc_head.get_kl()
        kl += self.fc_combine.get_kl()
        return kl
    ...
    def set_dropout(self,dropout):
        ...
        self.enc_ball.dropout = dropout
        self.enc_head.dropout = dropout
        self.fc_combine.dropout = dropout
