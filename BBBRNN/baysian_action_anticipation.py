...
class BActAnticipationModel(nn.Module):
    ...
    def __init__(self, data_type, input_size, output_size, hidden_dim,n_layers, rnn_dropout = 0.5, fc_dropout = 0.5, logstd = -2 ):
        super(BActAnticipationModel, self).__init__()
        linear_args =  {"logstd":logstd, "BBB":True}
        rnn_args = {"logstd":logstd, "BBB":True,"sharpen":True}
        ...
        self.sharpen = False
        self.lstm = bl.BaysianRNN(mode = "lstm", input_size = self.input_size,\
        ...
        self.fc_combine = bl.BaysianLinear(self.input_size, self.input_size, **linear_args)
        self.combine = nn.Sequential(self.fc_combine, nn.ReLU())
        
        # linear and sigmoid layers
        self.fc = bl.BaysianLinear(self.hidden_dim, self.output_size, **linear_args)
        self.dp_training = True
        self.baysian_layers = [self.context, self.movement,self.lstm,self.fc_combine,self.fc]
    ...
    def get_loss(self, output, targets):
        ...
        for layer in self.baysian_layers:
            if layer.BBB:
                KL += layer.get_kl()

        # KL divergence between posterior and sharpened distribution
        #this term is calculate only with respect to LSTM weights
        if self.sharpen:
            KL_sharp = self.lstm.get_kl_sharpening()
        else:
            KL_sharp = 0.
            ...
            for (video, interval) in videos:
                ...
                for sample in range(mc_samples):
                    ...
                    for i, data in enumerate(video):
                        ...
                        probs[i,sample] = F.log_softmax(out,1).exp().detach().numpy()
    ...
    def train(self, args, model, train_data,max_clip_sequence,device, log):
        ...
        for epoch in range(epochs):
            ...
            for data, label,end_sequence_batch in self.to_batch(train_data,seq=sequence, batch_size = batch, max =max_clip_sequence):
                ...
                NLL, KL, KL_sharp = model.get_loss(out, label)
                # proper scaling for sequence loss
                NLL_term = NLL / C
                # proper scaling for sequence loss
                KL_term = KL / B*C
                KL_sharp_term = KL_sharp 
                #Composed loss
                kl_term = scale*(KL_term + KL_sharp_term)
